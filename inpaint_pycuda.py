...
def tv_inpaint(f, mask, maxiter=1000, vis=False):
    ...
    while k < maxiter:
        ...
        if (vis and (k % 50 == 0)):
            ...
            clf()
            imshow(clip(u_vis), cmap=cm.gray)
            draw()
            ioff()
...
def tgv_inpaint(f, mask, fac=2.0, maxiter=1000, vis=False):
    ...
    while k < maxiter:
        ...
        if (vis and (k % 500 == 0)):
            ...
            cuda.memcpy_dtoh(u_vis.data, u.gpudata)
            clf()
            imshow(clip(u_vis.squeeze()), cmap=cm.gray)
            draw()
            ioff()
...
def clip(u,min=0.0,max=1.0):
    """ Convenience function: clips u to the values min and max."""
    return(minimum(maximum(u,min),max))
